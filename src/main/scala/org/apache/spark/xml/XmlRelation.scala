...
import org.apache.spark.xml.readers.{BulkXmlReader, LineXmlReader}
import org.apache.spark.xml.util._
...
case class XmlRelation protected[spark] (
    ...
    inferXmlSchema: Boolean)(@transient val sqlContext: SQLContext)
  ...
  private def tokenRdd(header: Array[String]): RDD[Array[String]] = {
    ...
    if (ParserLibs.isUnivocityLib(parserLib)) {
      univocityParseXML(baseRDD(), header)
    } else {
      ...
      baseRDD().mapPartitions { iter =>
        ...
        val xmlIter = if (useHeader) {
          iter.filter(_ != filterLine)
        } else {
          iter
        }
        parseXML(xmlIter, xmlFormat)
      }
    }
  }
    ...
    if (this.userSchema != null) {
      ...
      val firstRow = if (ParserLibs.isUnivocityLib(parserLib)) {
        ...
        val xmlFormat = defaultXmlFormat
          ...
          .withEscape(escape)
          .withSkipHeaderRecord(false)
        XMLParser.parse(firstLine, xmlFormat).getRecords.head.toArray
      }
  ...
  private def parseXML(
      ...
      xmlFormat: XMLFormat): Iterator[Array[String]] = {
    iter.flatMap { line =>
      try {
        val records = XMLParser.parse(line, xmlFormat).getRecords
        if (records.isEmpty) {
  ...
  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    ...
    if (overwrite) {
      ...
      data.saveAsXmlFile(filesystemPath.toString, Map("delimiter" -> delimiter.toString))
    } else {
